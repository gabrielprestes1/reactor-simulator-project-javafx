...
public class GraphFormController {
    ...
    @FXML
    private LineChart<?, ?> chartCon = new LineChart<>(new NumberAxis(), new NumberAxis());
    ...
    public void setResults(List<List<Double>> reults, List<String> equacoes, Integer particoes, Reactor reactor){
        ...
        for (int i = 0; i < reults.get(reults.size() - 1).size() / (equacoes.size()); i++) {
            ...
            Double exp = Double.parseDouble(formatter.format(i*reactor.getFinalTime() / Double.valueOf(particoes)));
        ...
        chartCon.getData().addAll(caXZ, cbXZ, ccXZ, cdXZ);
